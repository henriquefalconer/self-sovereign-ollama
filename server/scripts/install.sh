...
ACL_EOF
    echo ""
    echo "  Then:"
    echo -e "    • Tag this server: ${GREEN}tag:private-ai-server${NC}"
    echo -e "    • Tag client machines: ${GREEN}tag:ai-client${NC}"
    echo ""
    section_break
    echo "┌─────────────────────────────────────────────────────────────┐"
...
if [[ -n "$TAILSCALE_IP" ]]; then
    ...
    echo ""
    section_break
    echo -e "${BOLD}COMPLETE THE 3 STEPS ABOVE TO FINISH SETUP${NC}"
    echo ""
    ...
    echo -e "  • Test from client: ${BLUE}curl http://private-ai-server:11434/v1/models${NC}"
    echo ""
else
